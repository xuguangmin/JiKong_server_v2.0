...
void sum(FILE *fd, const char *name, char *sum_buffer, int buf_size)
{
	byte *buf;
	byte digest[16];
	char pr64[25];
	...
	memset(sum_buffer, 0, buf_size);
	if(hex)
	{
		for(i=0;i<16;i++) //printf("%.2X", digest[i]);
		{
			int ix = strlen(sum_buffer);
			sprintf(sum_buffer+ix, "%.2X", digest[i]);
		}
...
int calcu_md5_checksum(const char *filename, char *sum_buffer, int buf_size)
{
	...
	sum(fd, filename, sum_buffer, buf_size);
	fclose(fd);
	printf("%s\n", sum_buffer);
	return 1;
}
